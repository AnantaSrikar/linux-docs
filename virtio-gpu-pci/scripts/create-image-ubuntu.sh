...
DEBOOTSTRAP_PARAMS="--arch=$DEBARCH --components=main,universe --include=$PREINSTALL_PKGS $RELEASE $DIR"
...
network:
  ...
  ethernets:
    eth0:
      dhcp4: true
EOF
sudo chmod 600 "$DIR/etc/netplan/01-netcfg.yaml"

echo '/dev/root / ext4 defaults 0 0' | sudo tee -a "$DIR/etc/fstab"
printf "127.0.0.1\tlocalhost\n" | sudo tee "$DIR/etc/hosts"
echo "nameserver 8.8.8.8" | sudo tee -a "$DIR/etc/resolv.conf"
echo "$HOSTNAME" | sudo tee "$DIR/etc/hostname"
